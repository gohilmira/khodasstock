...
$taka = $this->uri->segment(3);
$Grey = $this->db->query("select g.Grey_Purchase_Order_Bill_Id,g.Taka,l.LName from grey_purchase_order_bill_details as g left join ledger as l on g.Mill=l.Ledger_Id where g.Card_No = '$taka'")->row_array();

$Grey_Purchase_Order_Bill_Id = $Grey['Grey_Purchase_Order_Bill_Id'];
$mill=$Grey['LName'];
$printPurchasebillData = $this->db->query("
	SELECT `gp`.`greypurchaseorderBillID`, `gp`.`GreyPNetAmt`, `gq`.`GreyQuality`, `gp`.`GreyPHsnCode`, `gp`.`GreyPSrNo`, `gp`.`GreyPRecMts`, `gp`.`GreyPBillNo`, `gp`.`GreyPPurchaseOrderBillDate`, `gp`.`GreyPPartyGstin`, `gp`.`GreyPPurRate`,`gp`.`GreyPCgstAmt`,`gp`. `GreyPSgstAmt`,`gp`.`GreyPBillAmt`, `l`.`LName`, `company_manager`.`CompanyID`, `company_manager`.`CompanyName` as `companyname`, `company_manager`.`CompanyGstInUin`, `company_manager`.`CompanyPanNo`, `company_manager`.`CompanyAddress1` as `companyaddress`, `l`.`LAddress`, `company_manager`.`AddressCompany` FROM `greypurchaseorderbill` as `gp`
	LEFT JOIN `grey_quality` as `gq` ON `gq`.`QualityID`=`gp`.`QualityID` 
	LEFT JOIN `ledger` as `l` ON `l`.`Ledger_Id`=`gp`.`GreyPPartyAcID` 
	LEFT JOIN `company_manager` ON `company_manager`.`CompanyID`=`gp`.`CompanyId` 
	WHERE `gp`.`greypurchaseorderBillID` = '$Grey_Purchase_Order_Bill_Id'")->row_array();
...
$PurRate=$printPurchasebillData['GreyPPurRate'];
$billamt=$printPurchasebillData['GreyPBillAmt'];
$cgstamt=$printPurchasebillData['GreyPCgstAmt'];
$sgstamt=$printPurchasebillData['GreyPSgstAmt'];
    ...
    <div class="container-fluid">
		<div class="container" id="printData">
			...
			<div class="row">
				...
				<table style="margin-bottom: 0rem; width: 100%;" class="table table-boarder">
					<tbody>
						<td style="text-align: left;border-top: unset;">TO, <?=$mill;?></td>
				...
				<?php
				$billdata = $this->db->query("SELECT * from greyptakadetails where TakaCardNo = '$taka'")->result();
				 $totalmts = 0;
				 $totaltaka = 0;
				...
				foreach ($billdata as $value)
				{
					 ...
					 echo '<td style="text-align:center;">'.$value->TakaMfgMts.'</td>';

					 $totalmts = $totalmts+($value->TakaMfgMts);
			...
			<table style="margin-bottom: 0rem; width: 100%;border-style: dashed;border-width: 0 0 1px 0; ">
				<tr>
					...
					<td colspan="4" style="padding-bottom: 9px;padding-left: 30px;">TOTAL MTS. &nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $totalmts; ?></td>
					<td colspan="2" style="padding-bottom: 9px;padding-left: 32px;">RATE :<?php echo $PurRate; ?></td>
					<!-- <td colspan="2" style="padding-bottom: 9px;padding-left: 33px;">VALUE :<?php echo intval($TotalMts*$PurRate); ?></td> -->
					<td colspan="2" style="padding-bottom: 9px;padding-left: 33px;">VALUE :<?php echo $PurRate*$totalmts; ?></td>
				</tr>
			</table>
			<table style="margin-bottom: 0rem; width: 100%;border-style: dashed;border-width: 0 0 1px 0; ">
				<tr>
					<td colspan="2" style="padding-bottom: 9px;"> CGST AMT :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $cgstamt; ?></td>
					<td colspan="4" style="padding-bottom: 9px;padding-left: 30px;">SGST AMT : &nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $sgstamt; ?></td>
					<!-- <td colspan="2" style="padding-bottom: 9px;padding-left: 20px;">SGST AMT :<?php //echo intval($PurRate); ?></td> -->
					<!-- <td colspan="2" style="padding-bottom: 9px;padding-left: 33px;">VALUE :<?php echo intval($TotalMts*$PurRate); ?></td> -->
					<td colspan="2" style="padding-bottom: 9px;padding-left: 33px;">Bill AMT :<?php echo $billamt; ?></td>
				</tr>
			</table>
			<div class="row">
